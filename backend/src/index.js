...
app.post('/api/orders', async (req, res) => {
    ...
    try {
        const order = await exports.prisma.order.create({
            data: {
        ...
        await exports.prisma.position.create({
            data: {
                user_id: DEMO_USER_ID,
                symbol: symbol,
                side: side,
...
app.get('/api/positions', async (req, res) => {
    const positions = await exports.prisma.position.findMany({ where: { user_id: DEMO_USER_ID } });
...
app.get('/api/history', async (req, res) => {
    const history = await exports.prisma.trade.findMany({ where: { user_id: DEMO_USER_ID }, orderBy: { date: 'desc' } });
    res.json(history);
});
